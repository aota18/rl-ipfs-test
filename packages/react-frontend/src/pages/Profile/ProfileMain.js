...
import CollectedSBTList from '../../components/sbt/SBTList';
...
const ProfileMain = () => {
  ...
  const { toggleTab, service } = useContext(SidebarContext);
  const {
    state: { user },
  ...
  const { data: sbtItems, loading: sbtLoading } = useAsync(() => {
        ...
        {service.id === 0 ? (
          <>
            <div className="flex space-x-4">
              ...
              <div className="flex flex-col justify-start border-r border-gray-300 pr-8">
                <span className="font-bold">
                  {sbtItems.constructor === Object && sbtItems.items.length}
                </span>
            ...
            <div className="space-y-4">
              ...
              <CollectedSBTList items={sbtItems.items} loading={sbtLoading} />
            </div>
